...
void Context::DrawFrame()
{
	...
	VkSemaphore wait_semaphores[] = { SyncObjects[CurrentFrame].ImageAvailableSemaphores };
	...
	VkSemaphore signal_semaphores[] = { SyncObjects[CurrentFrame].RenderFinishedSemaphores };
...
Expected<VkInstance> Context::CreateInstance()
{
	...
	for ( const auto& ext : ContextInfo.Extensions )
	{
		...
		{
			std::string message = std::format(
				"[Vulkan] Required extension is not available. Extension name: %s",
				ext );
			return std::unexpected( message );
		}
	}
...
Expected<std::vector<VulkanSyncObjects>> Context::CreateSyncObjects()
{
	...
	for ( VulkanSyncObjects& obj : sync_objs )
	{
		...
		{
			std::string message = std::format(
				"[Vulkan] Failed to create Render Finished semaphore object. vkCreateSemaphore returned {}={}",
				VK_TYPE_TO_STR( VkResult ),
				err );
			return std::unexpected( message );
		}
...
Expected<VulkanBuffer> Context::CreateIndexBuffer()
{
	...
	VulkanBuffer index_buffer = index_buffer_result.value();
