...
using VulkanExpected = std::expected<VkType, std::string>;
...
private:
	VulkanExpected<VkInstance>       CreateInstance();
	VulkanExpected<VkSurfaceKHR>     CreateSurface();
	VulkanExpected<VkPhysicalDevice> SelectPhysicalDevice();
	VulkanExpected<VkDevice>         CreateDevice( QueueFamilyIndices indices );
	VkQueue          GetQueue( uint32 family_index, uint32 index );

	VulkanExpected<VulkanSwapchain>  CreateSwapchain();
	void CleanupSwapchain();
	void RecreateSwapchain();

	VkShaderModule         CreateShaderModule( const std::vector<char>& code );
	VulkanExpected<VulkanGraphicsPipeline> CreateGraphicsPipeline( VkShaderModule vertex, 
		VkShaderModule fragment );

	VulkanExpected<std::vector<VkImageView>>   CreateImageViews();
	VulkanExpected<std::vector<VkFramebuffer>> CreateFramebuffers();

	VulkanExpected<VkCommandPool> CreateCommandPool( QueueFamilyIndices indices );
	std::vector<VkCommandBuffer> CreateCommandBuffers();

	VulkanExpected<std::vector<VulkanSyncObjects>> CreateSyncObjects();

	uint32 FindMemoryType( uint32 type_filter, VkMemoryPropertyFlags prop_flags );

	VulkanExpected<VulkanBuffer> CreateBuffer( VkDeviceSize size, VkBufferUsageFlags usage,
		VkMemoryPropertyFlags props );
	VulkanExpected<VulkanBuffer> CreateVertexBuffer();
	VulkanExpected<VulkanBuffer> CreateIndexBuffer();
	VulkanExpected<std::vector<VulkanBuffer>> CreateUniformBuffers();
	...
	VulkanRHI::Texture CreateTexture();
	VulkanRHI::Texture CreateTextureImage( int32 width, int32 height, VkFormat format,
	...
	void TransitionImageLayout( const VulkanRHI::Texture& texture, VkFormat format,
...
private:
	...
	VulkanRHI::Texture DepthTexture;
